...
impl ZipFile {
    ...
    pub fn open(mut file: Cursor<Vec<u8>>) -> Result<Self> {
    ...
    pub fn copy_all_entries_to(&mut self, dst_archive: &mut ZipFile) -> Result<()> {
        let mut buf_reader = BufReader::new(&mut self.file);
    ...
    fn copy_entry_internal(
        ...
        buf_reader: &mut BufReader<&mut Cursor<Vec<u8>>>,
        dst_name: String,
        dst_archive: &mut ZipFile,
    ) -> Result<()> {
...
impl ZipFile {
    ...
    pub fn save_and_sign_v2(&mut self, priv_key: &RsaPrivateKey, cert: &Certificate) -> Result<()> {
        ...
        self.file
            .get_mut()
            .truncate(self.end_of_entries_offset as usize);
        // file.into_inner().into_inner().set_len(self.end_of_entries_offset as usize);
